...
// The dealer can't recall the information for a car with an id of 33 on his lot. Help the dealer find out which car has an id of 33 by logging the car's year, make, and model in the console log provided to you below:
 for (let i = 0; i < inventory.length; i++) {
     if (inventory[i].id === 33)
         console.log(` Car   ${inventory[i].id} is a ${inventory.car_year} ${inventory[i].car_make} ${inventory[i].car_model}`);
 }
...
console.log(inventory [lastCar].car_make, inventory[lastCar].carModels ) ;

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];
for (let i = 0; i < inventory.length; ii++)     {
    carModels .push (inventory[lastCar]  .car_model)
}
...
let BMWAndAudi = [];
for (let i = 0; i < inventory.length; i ++); {
    if (inventory[i].car_model === `BMw` || inventory[i].car_make === ` Audi `) {
